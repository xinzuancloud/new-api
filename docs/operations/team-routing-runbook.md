...
- 429 从基础冷却时间开始，连续触发时指数退避至配置上限；冷却状态保存在 Redis，成功请求会重置退避。匹配额度关键词时使用独立的额度冷却时间。关键词区分额度耗尽和鉴权失效；额度关键词从永久禁用列表移到临时冷却列表，鉴权禁用和原生恢复探测保留。
- 可按渠道标签配置滚动请求数和预计输入 Token 容量。网关在同一标签和渠道优先级内选择近期负载最低的账号；预计超过容量的账号暂时跳过。空闲账号仍允许一次超大请求，以免仅凭本地估算把所有账号误判为不可用。
...
选择顺序仍是分组供应商顺序；同账号优先原生协议，转换不会跨越白名单或替换模型。原生请求保留 DTO 尚未认识的顶层字段，映射模型、系统提示词、禁用字段和参数覆盖仍由网关控制。不兼容候选只记入 `admin_info.protocol_skips`，不消耗上游尝试次数。原生 Claude 上游若以 HTTP 200 返回零事件空流，网关在同一端点安全重试一次，每个客户端请求最多触发一次这种额外重试，再按原顺序换账号；已发送任何流式字节后禁止重试或切换供应商。流式失败发出客户端格式的错误事件，保留已报告的部分用量结算，不能在失败流后补成功完成事件。
