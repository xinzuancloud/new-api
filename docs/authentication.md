...
- `new_api_has_session` 是 Refresh Cookie 的会话提示，值恒为 `1`，`Path=/`、非 `HttpOnly`，与 Refresh Cookie 同时写入、同时清除、同一过期时间。它只声明"曾签发过 Refresh Cookie"，不含任何凭据，也不参与任何鉴权判定；伪造它唯一的效果是自费一次注定失败的 refresh。它存在的原因是 Refresh Cookie 被 `HttpOnly` 和 `Path=/api/user/auth` 双重限制，`/` 上的页面无法判断自己是否匿名，否则每次冷启动都要发一次注定 401 的 refresh，并占用 refresh 的独立 IP 限流额度。
...
前端将冷启动状态与登录状态分开管理。根路由完成的会话检查会传给本次导航的嵌套路由复用，避免受保护路由和登录页连续重复 refresh。网络、429 或服务端临时故障会让受保护路由显示错误并保留可重试状态，登录页仍可使用独立的登录额度；只有服务端确认 Refresh Cookie 无效时才进入已完成的匿名状态并跳转登录页。内存 SID 与 Cookie SID 不一致时，客户端清除旧内存身份并在不携带旧 SID 的情况下重试一次。
...
Redis 限流使用原子 Lua 固定窗口，替代旧的近似滑动窗口 List 实现。窗口边界两侧可分别打满一次，极短时间内通过量最高约为配置值的两倍。匿名登录的密码、2FA、登录验证和 Passkey 各阶段共享 `AUTH_LOGIN_RATE_LIMIT` / `AUTH_LOGIN_RATE_LIMIT_DURATION`（默认 `60 次/1200 秒/IP`）；refresh 使用独立的 `AUTH_REFRESH_RATE_LIMIT` / `AUTH_REFRESH_RATE_LIMIT_DURATION`（默认 `120 次/1200 秒/IP`），不会消耗登录或其他 Critical 操作的额度。四项环境变量必须为正数，非法值会记录错误并回退默认值。帐户级 Session 上限和签发窗口继续控制数据库增长；如未来需要严格抑制边界突发，需单独迁移为 ZSET 滑动窗口。

该设计按 OWASP ASVS 5.0.0 的 V6.1.1 记录鉴权限流策略，以 V6.3.1 和 V2.4.1 的反自动化要求保留服务端限制，并按 V6.3.4 让所有匿名登录路径共享相同控制，避免共享 IP 的 refresh 流量锁死登录入口。验证覆盖独立限流命名空间、固定窗口上限和 `Retry-After`，以及前端仅在服务端确认 401 后进入匿名状态；没有改变密码校验、会话轮换、Origin/CSRF 防护或审计日志的敏感字段规则。
